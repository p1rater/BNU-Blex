 ...
 * BlexOS kernel – Copyright (C) 2026 Blex – BOSL License
 *
 * TTY mode: 1920×1080 framebuffer used as a single full-screen
 * terminal (no window manager).  Characters are drawn with the
 * built-in 8×16 bitmap font; the screen scrolls line-by-line
 * when the cursor reaches the bottom.
 *
 * FAT32: 512 KB in-memory ramdisk, unchanged from original.
 */
...
/* ── TTY cursor state ─────────────────────────────── */
static int tty_cx = 0;   /* column (chars) */
static int tty_cy = 0;   /* row    (chars) */
static uint32_t tty_fg  = COL_TEXT_FG;
static uint32_t tty_bg  = COL_SHELL_BG;

/* Derived at fb_init time */
static int tty_cols = 0;
static int tty_rows = 0;

/* UTF-8 decoder state */
static utf8_decoder_t utf8_dec;   // <-- EKLENDI (fb_init'ten önce)
...
/* ══════════════════════════════════════════════════════
 *  STANDARD 8×16 BITMAP FONT (VGA-style, ASCII 32–127)
 * ══════════════════════════════════════════════════════ */
/* ── Extended bitmap font for TTY (ASCII 32-255, ISO-8859-9 Turkish support) ── */
/* Index = character - 32. Last 128 slots include Turkish accented letters.       */
static const uint8_t font8x16[224][16] = {
    ...
    /* 0x25  %     */ {0,0,0,0,0xC6,0xCC,0x18,0x30,0x60,0xC6,0x86,0x0C,0,0,0,0,0},
    ...
    /* 0x2F  /     */ {0,0,0,0x03,0x06,0x0C,0x18,0x30,0x60,0xC0,0x80,0,0,0,0,0},
    ...
    /* Ç 0xC7 */ {0,0,0x3E,0x63,0xC0,0xC0,0xC0,0xC0,0xC0,0x63,0x3E,0x06,0x06,0,0,0},
    /* Ğ 0xD0 */ {0x3C,0x42,0x3E,0x63,0xC0,0xC0,0xCF,0xC3,0xC3,0x63,0x3E,0,0,0,0,0},
    /* Ö 0xD6 */ {0x24,0x24,0x7E,0xC3,0xC3,0xC3,0xC3,0xC3,0xC3,0xC3,0x7E,0,0,0,0,0},
    /* Ü 0xDC */ {0x24,0x24,0xC3,0xC3,0xC3,0xC3,0xC3,0xC3,0xC3,0xC3,0x7E,0,0,0,0,0},
    /* İ 0xDD */ {0x24,0x24,0x7E,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x7E,0,0,0,0,0},
    /* Ş 0xDE */ {0,0,0x7E,0xC3,0xC0,0x7E,0x03,0x03,0x03,0xC3,0x7E,0x0C,0x0C,0,0,0},
    /* ç 0xE7 */ {0,0,0,0,0x7C,0xC6,0xC0,0xC0,0xC0,0xC6,0x7C,0x0C,0x0C,0,0,0},
    /* ğ 0xF0 */ {0x3C,0x42,0,0,0x7E,0xC6,0xC6,0xC6,0x7E,0x06,0x7C,0,0,0,0,0},
    /* ö 0xF6 */ {0,0x24,0x24,0,0x7C,0xC6,0xC6,0xC6,0xC6,0xC6,0x7C,0,0,0,0,0},
    /* ü 0xFC */ {0,0x24,0x24,0,0xCC,0xCC,0xCC,0xCC,0xCC,0xCC,0x7E,0,0,0,0,0},
    /* ı 0xFD */ {0,0,0,0,0,0x38,0x18,0x18,0x18,0x18,0x3C,0,0,0,0,0},
    /* ş 0xFE */ {0,0,0,0,0x7C,0xC6,0x60,0x3C,0x06,0xC6,0x7C,0x0C,0x0C,0,0,0},
};

/* ── Draw one character (unsigned char, supports ISO-8859-9 chars 32-255) ── */
...
void fb_draw_char(int x, int y, char c, uint32_t fg, uint32_t bg) {
    tty_draw_glyph(x, y, (unsigned char)c, fg, bg);
}
...
void fb_init(uint32_t magic, void* mb2_info) {
    ...
    if (!fb.addr) {
        ...
        fb.pitch  = 1920 * 4;
        fb.bpp    = 32;
        fb.addr   = (uint32_t*)0xFD000000; /* typical VESA LFB */
    }

    /* Compute TTY grid dimensions */
    ...
    utf8_decoder_init(&utf8_dec);

    /* Blank the screen */
    fb_fill_rect(0, 0, (int)fb.width, (int)fb.height, tty_bg);
}
...
void tty_putchar(char c) {
    uint32_t codepoint;
    int complete = utf8_decode(&utf8_dec, (uint8_t)c, &codepoint);

    if (!complete) return;  // Waiting for more bytes
    ...
    if (codepoint == '\n') {
        ...
        int next_tab = ((tty_cx / 8) + 1) * 8;
        while (tty_cx < next_tab && tty_cx < tty_cols) {
    ...
    // Printable character - convert to font index
    int font_idx = unicode_to_font_index(codepoint);
    if (font_idx < 0 || font_idx >= 224) {
        font_idx = '?' - 32;  // Fallback to '?'
    }
    ...
    // Draw the glyph
    const uint8_t* glyph = font8x16[font_idx];
    int px = TTY_PAD_X + tty_cx * FONT_W;
    int py = TTY_PAD_Y + tty_cy * FONT_H;
...
void tty_print_prompt(const char* user, const char* host) {
    tty_print_color(user,   RGB(0, 255, 255));
    tty_print_color("@",    RGB(255, 255, 0));
    tty_print_color(host,   RGB(0, 255, 0));
    tty_print_color(":~# ", RGB(255, 255, 255));
}
...
void utf8_decoder_init(utf8_decoder_t* dec) {
    dec->codepoint = 0;
    dec->bytes_expected = 0;
    dec->bytes_received = 0;
    for (int i = 0; i < 4; i++) dec->buffer[i] = 0;
}
...
int utf8_decode(utf8_decoder_t* dec, uint8_t byte, uint32_t* out_codepoint) {
    if ((byte & 0x80) == 0x00) {
        ...
        *out_codepoint = byte;
        ...
        dec->bytes_expected = 2;
        dec->bytes_received = 1;
        dec->buffer[0] = byte;
        return 0;
    }
        ...
        if (dec->bytes_received == 0 || dec->bytes_received >= dec->bytes_expected) {
            // Invalid sequence
            dec->bytes_expected = 0;
            dec->bytes_received = 0;
        ...
        dec->buffer[dec->bytes_received++] = byte;

        if (dec->bytes_received == dec->bytes_expected) {
            *out_codepoint = dec->codepoint;
            dec->bytes_expected = 0;
            dec->bytes_received = 0;
            return 1;  // Complete character
        }
        return 0;
    }
    else {
...
/* ── Unicode Codepoint → Font Index (ISO-8859-9) ── */
int unicode_to_font_index(uint32_t codepoint) {
    // ASCII 0-127
    if (codepoint < 128) {
        if (codepoint < 32) return -1;
        return (int)codepoint - 32;
    }

    // Turkish characters (ISO-8859-9 / Latin-5)
    switch (codepoint) {
        case 0x00C7: return 167;   // Ç
        case 0x011E: return 176;   // Ğ
        case 0x00D6: return 182;   // Ö
        case 0x00DC: return 188;   // Ü
        case 0x0130: return 189;   // İ
        case 0x015E: return 190;   // Ş
        case 0x00E7: return 199;   // ç
        case 0x011F: return 208;   // ğ
        case 0x00F6: return 214;   // ö
        case 0x00FC: return 220;   // ü
        case 0x0131: return 221;   // ı
        case 0x015F: return 222;   // ş
        default: break;
    }

    // If codepoint is in 0x80-0xFF range (ISO-8859-1)
    if (codepoint >= 0x80 && codepoint <= 0xFF) {
        if (codepoint < 32) return -1;
        return (int)codepoint - 32;
    }

    // Unknown character: fallback to '?'
    return '?' - 32;
}
